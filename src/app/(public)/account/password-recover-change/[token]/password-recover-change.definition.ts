...
export type PasswordRecoverChangeSituationType = FormSituationType | 'csrf_error';

export type PasswordRecoverChangeStateType = {
	token: string;
	values: PasswordRecoverChangeFormFieldsType;
	errors: Partial<Record<keyof PasswordRecoverChangeFormFieldsType, string[]>>;
	message: string | null;
...
export const PasswordRecoverChangeSchema = z
	.object({
		password: z
			.string({
				message: translations['password_recover_change.validation.password_invalid'],
			})
			.trim()
			.min(cfg('user.passwordMinLength') as number, {
				message: translations['password_recover_change.validation.password_min'],
			})
