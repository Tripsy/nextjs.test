...
export async function getClientIp(headersProvided?: Headers): Promise<string | undefined> {
    ...
    let ip = forwardedIp
        ...
        : headersSource.get('cf-connecting-ip') || headersSource.get('x-real-ip');
...
export async function apiHeaders(headersProvided?: Headers): Promise<ApiHeaders> {
    const headersSource = headersProvided || await headers();
