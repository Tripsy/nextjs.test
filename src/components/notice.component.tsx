...
export function Notice({type = 'info', title, message, children}: NoticeProps) {
    switch (type) {
        case 'success':
            return <NoticeSuccess title={title} message={message}>{children}</NoticeSuccess>;
        case 'error':
            return <NoticeError title={title} message={message}>{children}</NoticeError>;
        case 'warning':
            return <NoticeWarning title={title} message={message}>{children}</NoticeWarning>;
        case 'info':
            return <NoticeInfo title={title} message={message}>{children}</NoticeInfo>;
        case 'loading':
            return <NoticeLoading title={title} message={message}>{children}</NoticeLoading>;
    }
}
    ...
    return (
        <div className="shadow-lg rounded-t-lg rounded-b-none">
            ...
            <div className="alert alert-soft alert-error rounded-none">
                <div>
                    <span>
                        <Icons.Error className="w-5 h-5"/> {message ?? 'Something went wrong!'}
                    </span>
                    { children }
                </div>
            </div>
    ...
    return (
        <div className="shadow-lg rounded-t-lg rounded-b-none">
            ...
            <div className="alert alert-soft alert-info rounded-none">
                <div>
                    <span>
                        <Icons.Loading className="w-5 h-5 animate-spin"/> {message ?? 'Loading...'}
                    </span>
                    { children }
                </div>
            </div>
